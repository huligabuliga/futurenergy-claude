...
import { PRONTO_TABLES, PRONTO_ENUMS, PRONTO_RPCS, LEAD_STATUS_VALUES, LEAD_STATUS_LABELS, LEAD_QUALIFICATION_VALUES, VENDOR_QUALIFICATION_VALUES, LEAD_PROGRAM_VALUES, CRM_ACTIVITY_TYPES, CRM_ACTIVITY_CATEGORIES, TICKET_PRIORITY_SLA_DAYS, PHOTO_TO_DOCUMENT_MAP, VT_TO_SF_FIELD_MAP, ACCOUNT_EMAIL_FIELDS, KEY_FIELDS, getRpcsByCategory, } from "./schema.js";
// ── Supabase connection ───────────────────────────────────
...
server.tool("futurerp_ticket_kpis", "Ticket health KPIs: open vs resolved counts, SLA breaches (priority drives deadline: alta=1d, media=2d, baja=3d laborales), breakdown by area, by status, and by responsable.", {
    ...
    try {
        ...
        const baseQs = filtersToQs(filters);
        const tickets = await sbApi(`/tickets?${baseQs ? baseQs + "&" : ""}select=id,folio,status,prioridad,canal,area,responsable,fecha_creacion,fecha_vencimiento&limit=5000`);
...
server.tool("futurerp_instalacion_kpis", "Field installation KPIs: status breakdown (asignada/pendiente/en_proceso/completada/cancelada), completion rate, total jobs in period, panels installed, and per-cuadrilla counts.", {
    ...
    try {
        ...
        const filters = [];
        if (range.gte)
            filters.push({ column: "fecha_programada", op: "gte", value: range.gte });
        if (range.lte)
            filters.push({ column: "fecha_programada", op: "lt", value: range.lte });
        const baseQs = filtersToQs(filters);
        const rows = await sbApi(`/instalaciones?${baseQs ? baseQs + "&" : ""}select=id,folio,estado,cuadrilla_id,paneles,fecha_programada,checkin_at,checkout_at&limit=5000`);
