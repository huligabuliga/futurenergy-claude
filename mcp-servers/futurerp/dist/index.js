...
const invalidCache = new Map(); // token → until (negative cache: don't re-ask Supabase for 60 s)
...
export function buildServer(auth) {
    ...
    server.tool("futurerp_download_file", "Fetch a file by its public URL (Firebase Storage public URL or Google Drive web_content_link) and return its contents. Images (jpeg/png/gif/webp) and PDFs up to 3.5 MB come back inline so you can actually look at them; text/JSON/XML up to 200 KB comes back as text; anything bigger or of another type returns metadata + the URL.", {
        ...
        try {
            ...
            if (!allowed)
                throw new Error(`Host not allowed: ${host} (only Firebase Storage / Google Drive / Supabase URLs)`);
            const res = await fetch(url);
            if (!res.ok) {
            ...
            const header = () => ({ type: "text", text: lines.join("\n") });
            ...
            if (buf.length <= MAX_INLINE_BYTES && INLINE_IMAGE_MIMES.has(mime)) {
                return {
                    content: [header(), { type: "image", data: buf.toString("base64"), mimeType: mime }],
                };
            }
            if (buf.length <= MAX_INLINE_BYTES && mime === "application/pdf") {
                return {
                    content: [
                        header(),
                        { type: "resource", resource: { uri: url, mimeType: mime, blob: buf.toString("base64") } },
                    ],
                };
            }
            lines.push(``, buf.length > MAX_INLINE_BYTES
...
async function main() {
    ...
    app.use((req, res, next) => {
        const t0 = Date.now();
        res.on("finish", () => {
            const line = { m: req.method, p: req.path, s: res.statusCode, ms: Date.now() - t0 };
            if (req.path === "/mcp" && req.body?.method)
                line.rpc = req.body.method;
            const wa = res.getHeader("www-authenticate");
            if (typeof wa === "string")
                line.reason = wa.replace(/, resource_metadata=.*$/, "");
