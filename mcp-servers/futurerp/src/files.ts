...
export function sniffMime(buf: Buffer, declared: string): string {
  ...
  for (const [sig, mime] of magic) if (head.startsWith(sig)) return mime;
  if (head === "52494646" && buf.subarray(8, 12).toString("ascii") === "WEBP") return "image/webp";
