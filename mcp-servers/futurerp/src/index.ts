 ...
 */
async function cuadrillaNames(ids?: string[]): Promise<Map<string, string>> {
  const wanted = ids?.filter((id) => UUID_RX.test(id));
  if (ids && (!wanted || wanted.length === 0)) return new Map();
  const filter = wanted ? `id=in.(${wanted.join(",")})&` : "";
  const rows = await sbApi(`/cuadrillas?${filter}select=id,nombre&limit=1000`).catch(() => []);
  return new Map((Array.isArray(rows) ? rows : []).map((r: any) => [r.id as string, r.nombre as string]));
}

function cuadrillaLabel(id: string | null | undefined, names: Map<string, string>): string {
 ...
 */
async function resolveProfiles(authUuids: string[]): Promise<Map<string, string>> {
  const out = new Map<string, string>();
  const valid = [...new Set(authUuids)].filter((u) => u && UUID_RX.test(u));
  if (valid.length === 0) return out;
  try {
    const rows = await sbApi(
      `/profiles?user_id=in.(${valid.map((u) => encodeURIComponent(u)).join(",")})&select=user_id,first_name,last_name,email`
    );
    ...
    if (!category && hasCredentials()) {
      try {
        ...
        let extras = Object.keys(spec.definitions ?? {})
          ...
          .sort();
        if (filter) extras = extras.filter((n) => n.toLowerCase().includes(filter.toLowerCase()));
        if (extras.length) {
          liveCount = extras.length;
          sections.push(
            `### (uncatalogued — live from DB) (${extras.length})\n\n${extras
...
server.tool(
  ...
  async ({ table, id, id_column, select }) => {
    try {
      ...
      const row = rows[0];
      const composedName =
        row.first_name || row.last_name ? [row.first_name, row.last_name].filter(Boolean).join(" ") : null;
      const label =
        row.folio ??
        row.efu ??
        row.name ??
...
server.tool(
  ...
  "Field installation KPIs: status breakdown (asignada/pendiente/en_proceso/completada/cancelada), completion rate, total jobs in period, panels installed, and per-cuadrilla counts (by crew name, from the cuadrillas table).",
  {
    period: z
      .enum(["this_month", "last_month", "this_quarter", "last_quarter", "this_year", "last_year", "all_time"])
      .optional()
  ...
  async ({ period }) => {
    try {
      ...
      const rows: any[] = await sbApi(
        `/instalaciones?${baseQs ? baseQs + "&" : ""}select=id,efu,status,cuadrilla_id,numero_paneles,fecha_instalacion&limit=5000`
      );
      ...
      const cuadrillaRows = topCuadrillas
        .map(
          ([k, s]) =>
            `| ${names.get(k) ?? (UUID_RX.test(k) ? "(sin nombre)" : k)} | ${UUID_RX.test(k) ? k : "—"} | ${s.total} | ${s.completed} | ${
  ...
  async ({ lead_id, activity_limit }) => {
    try {
      ...
      const [activities, history, project] = await Promise.all([
        ...
        sbApi(
          `/lead_stage_history?lead_id=eq.${encodeURIComponent(lead_id)}&select=*&order=entered_at.desc&limit=20`
        ).catch(() => []),
      ...
      if (lead.co_owner_id) userIds.add(lead.co_owner_id);
      if (lead.created_by) userIds.add(lead.created_by);
      for (const a of Array.isArray(activities) ? activities : []) if (a.performed_by) userIds.add(a.performed_by);
      const profileMap = await resolveProfiles([...userIds]);

      const fmtUser = (u?: string | null) => (u && profileMap.get(u)) || u || "—";

      const enrichedActivities = (Array.isArray(activities) ? activities : []).map((a: any) => ({
        ...a,
...
server.tool(
  ...
  async ({ entity_type, parent_id, limit }) => {
    try {
      ...
      const data = await sbApi(
        `/${spec.table}?${encodeURIComponent(spec.fk)}=eq.${encodeURIComponent(parent_id)}&select=${encodeURIComponent(cols)}&order=${createdCol}.desc&limit=${lim}`
      );
      const rows: any[] = Array.isArray(data) ? data : [data];
      if (rows.length === 0) {
        return {
...
server.tool(
  ...
  async ({ ticket_id, activity_limit }) => {
    try {
      ...
      for (const u of t.responsables_ids ?? []) if (u) userIds.add(u);
      for (const a of Array.isArray(activities) ? activities : []) if (a.user_id) userIds.add(a.user_id);
      const profileMap = await resolveProfiles([...userIds]);
      const fmtUser = (u?: string | null) => (u && profileMap.get(u)) || u || "—";

      const responsablesDisplay =
        Array.isArray(t.responsables_ids) && t.responsables_ids.length > 0
          ? t.responsables_ids.map((u: string) => fmtUser(u)).join(", ")
  ...
  async ({ project_id }) => {
    try {
      ...
      const [files, tramites, payments, instalaciones, expenses, client] = await Promise.all([
        ...
        sbApi(
          `/instalaciones?project_id=eq.${encodeURIComponent(p.id)}&select=id,efu,tipo_servicio,status,fecha_instalacion,cuadrilla_id,numero_paneles&order=fecha_instalacion.desc&limit=20`
        ).catch(() => []),
        sbApi(
      ...
      const instRowsRaw: any[] = Array.isArray(instalaciones) ? instalaciones : [];
      const instCrewNames = await cuadrillaNames(
        [...new Set(instRowsRaw.map((r) => r.cuadrilla_id).filter(Boolean))] as string[]
      );
    ...
    try {
      ...
      const [fotos, lineItems, reports, visits, crew, crewNames] = await Promise.all([
        ...
        Array.isArray(inst.assigned_to) && inst.assigned_to.length > 0
          ? sbApi(
              `/profiles?user_id=in.(${inst.assigned_to.map((u: string) => encodeURIComponent(u)).join(",")})&select=user_id,first_name,last_name,email,department`
            ).catch(() => [])
          : Promise.resolve([]),
      ...
      const crewWithNames = crewRows.map((p: any) => ({
        ...p,
...
//
// Data model (written by the bot Edge Functions in pronto-resolver-61):
// - Pre-lead capture: whatsapp_pending_contacts.messages jsonb [{dir:'in'|'out', text, ts}]
// - Lead phase: crm_activities rows (entity_type='lead', activity_type='whatsapp');
//   metadata: direccion in|out, canal (present ⇒ bot-logged), mensaje_cliente (inbound text),
//   mensaje_bot (reply text), status='agente' ⇒ a human vendor sent it via the app.
//   ONE row can carry both an inbound message and the bot's reply.
// - Errors: bot_error_log (fase × tipo) + webhook_outbox target='whatsapp' failures.
// ────────────────────────────────────────────────────────────
...
const WA_CHANNEL_LABELS: Record<string, string> = {
  whatsapp: "WhatsApp",
  instagram: "Instagram",
  facebook: "Messenger",
  tiktok: "TikTok",
...
function pendingMsgCounts(messages: any): { in: number; out: number; last?: string } {
  const arr: PendingMsg[] = Array.isArray(messages) ? messages : [];
  let inN = 0,
    outN = 0,
    last: string | undefined;
  for (const m of arr) {
    if (m?.dir === "in") inN++;
    else if (m?.dir === "out") outN++;
    if (m?.ts && (!last || m.ts > last)) last = m.ts;
  }
  return { in: inN, out: outN, last };
}

const fmtTs = (v?: string | null) => (v ? String(v).slice(0, 16).replace("T", " ") : "—");
const clip = (v: any, n = 60) => {
...
server.tool(
  ...
  "Overview of WhatsApp/Messenger/Instagram/TikTok bot conversations: pre-lead pending contacts (with in/out message counts, triage, lifecycle status) and leads with recent bot activity (last inbound/outbound, escalation, intent). Use futurerp_whatsapp_conversation to open a full thread.",
  {
  ...
  async ({ source, channel, days, limit, only_escalated, status }) => {
    try {
      ...
      if (src !== "leads") {
        ...
        if (channel) qs += `&channel=eq.${channel}`;
        if (status) qs += `&status=eq.${status}`;
        const rows: any[] = await sbApi(`/whatsapp_pending_contacts?${qs}`);
      ...
      if (src !== "pending") {
        ...
        if (only_escalated) qs += `&bot_escalation_reason=not.is.null`;
        const rows: any[] = await sbApi(`/leads?${qs}`);
        const table = rows.map((r) => ({
...
server.tool(
  ...
  "Full message thread for one WhatsApp bot contact — capture-phase messages (whatsapp_pending_contacts) plus lead-phase messages (crm_activities), rendered chronologically with direction and sender (bot / agente / cliente). Identify the contact by lead_id, respond_contact_id, or phone.",
  {
  ...
  async ({ lead_id, respond_contact_id, phone, limit }) => {
    try {
      ...
      const pendingSelect =
        "select=respond_contact_id,channel,phone,contact_name,captured,tipo_contacto,status,status_reason,lead_id,messages,nudges,created_at,updated_at";

      if (phone && !lead_id && !respond_contact_id) {
        const digits = phone.replace(/\D/g, "").slice(-10); // last 10 digits — MX local
        const leads: any[] = await sbApi(
          `/leads?mobile_phone_e164=ilike.${encodeURIComponent(`*${digits}*`)}&select=*&limit=2`
        );
        lead = leads[0] ?? null;
        if (!lead) {
          const pend: any[] = await sbApi(
            `/whatsapp_pending_contacts?phone=ilike.${encodeURIComponent(`*${digits}*`)}&${pendingSelect}&limit=2`
          );
          pending = pend[0] ?? null;
        }
        if (!lead && !pending) throw new Error(`No lead or pending contact matches phone ${phone}.`);
      }

      if (respond_contact_id) {
        const pend: any[] = await sbApi(
          `/whatsapp_pending_contacts?respond_contact_id=eq.${encodeURIComponent(respond_contact_id)}&${pendingSelect}`
        );
        pending = pend[0] ?? null;
        if (!pending) throw new Error(`No pending contact ${respond_contact_id}.`);
        if (pending.lead_id) lead_id = pending.lead_id;
      }

      if (lead_id && !lead) {
        const rows: any[] = await sbApi(`/leads?id=eq.${encodeURIComponent(lead_id)}&select=*`);
        lead = rows[0] ?? null;
        if (!lead) throw new Error(`No lead ${lead_id}.`);
      }
      // A promoted lead's capture conversation lives on its pending row — merge it in.
      if (lead && !pending) {
        const pend: any[] = await sbApi(
          `/whatsapp_pending_contacts?lead_id=eq.${lead.id}&${pendingSelect}&limit=1`
        );
        pending = pend[0] ?? null;
      }
      ...
      if (lead) {
        header.push(
          `**Lead:** ${[lead.first_name, lead.last_name].filter(Boolean).join(" ") || lead.id} (\`${lead.id}\`)`,
          `**Tel:** ${lead.mobile_phone_e164 ?? lead.phone ?? "—"} · **Etapa:** ${lead.status} · **Bot:** ${lead.whatsapp_bot_active === false ? "apagado" : "activo"}`
        );
        if (lead.intent_detected || lead.interest_level)
          header.push(`**Intent:** ${lead.intent_detected ?? "—"} · **Interés:** ${lead.interest_level ?? "—"}`);
        if (lead.bot_escalation_reason) header.push(`**Escalado:** ${lead.bot_escalation_reason}`);
        if (lead.bot_conversation_summary) header.push(`**Resumen del bot:** ${lead.bot_conversation_summary}`);
      }
      if (pending) {
        header.push(
          `**Pending contact:** ${pending.contact_name ?? pending.phone ?? pending.respond_contact_id} (\`${pending.respond_contact_id}\`) · **Canal:** ${WA_CHANNEL_LABELS[pending.channel] ?? pending.channel} · **Triage:** ${pending.tipo_contacto} · **Status:** ${pending.status}${pending.status_reason ? ` (${pending.status_reason})` : ""}`
        );
        const capturedKeys = Object.keys(pending.captured ?? {});
        if (capturedKeys.length) header.push(`**Capturado:** ${capturedKeys.join(", ")}`);
      }
      ...
      const pendMsgs: PendingMsg[] =
        !capturaReplayed && Array.isArray(pending?.messages) ? pending.messages : [];
      if (pendMsgs.length) {
      ...
      if (lead) {
        const acts: any[] = await sbApi(
          `/crm_activities?entity_type=eq.lead&entity_id=eq.${lead.id}&activity_type=eq.whatsapp&order=created_at.asc&select=id,description,metadata,created_at&limit=${lim}`
        );
        if (acts.length) {
          ...
          for (const a of acts) {
            ...
            const emitted: boolean = !!(meta.mensaje_cliente || meta.mensaje_bot);
            if (meta.mensaje_cliente) lines.push(`- ${ts} ⬅️ **cliente**: ${clip(meta.mensaje_cliente, 500)}`);
            if (meta.mensaje_bot) {
          ...
          if (acts.length === lim) lines.push("", `*Truncado a ${lim} actividades — sube \`limit\` para ver más.*`);
        }
      }
      if (!lines.length) lines.push("*Sin mensajes.*");
...
server.tool(
  ...
  "Inbound vs outbound WhatsApp bot message stats for reporting: totals (in / out-bot / out-agente), breakdown by channel and by day/week, active conversations, new contacts, promotions to lead, triage mix, and escalations. Combines lead-phase crm_activities with capture-phase pending-contact messages.",
  {
    period: z
      .enum(["this_month", "last_month", "this_quarter", "last_quarter", "this_year", "last_year", "all_time"])
      .optional()
  ...
  async ({ period, channel, group_by }) => {
    try {
      requireCredentials();
      const range = periodToRange(period ?? "this_month");
      const bucketOf = (ts: string): string => {
        const day = ts.slice(0, 10);
        if ((group_by ?? "day") === "day") return day;
        const d = new Date(day + "T00:00:00Z");
        d.setUTCDate(d.getUTCDate() - ((d.getUTCDay() + 6) % 7)); // Monday of week
        return `sem. ${d.toISOString().slice(0, 10)}`;
      };
      const inRange = (ts?: string) =>
        !!ts && (!range.gte || ts >= range.gte) && (!range.lte || ts < range.lte);

      // Tallies
      let inbound = 0,
        outBot = 0,
        outAgente = 0;
      const byChannel: Record<string, { in: number; out: number }> = {};
      const byBucket: Record<string, { in: number; out: number }> = {};
      const tally = (ch: string, dir: "in" | "out", ts: string) => {
      ...
      if (range.gte) qs += `&created_at=gte.${range.gte}`;
      if (range.lte) qs += `&created_at=lt.${range.lte}`;
      if (channel) qs += `&metadata->>canal=eq.${channel}`;
      const acts: any[] = await sbApi(`/crm_activities?${qs}`);
      ...
      for (const a of acts) {
        ...
        if (meta.mensaje_bot || (meta.direccion === "outbound" && !meta.mensaje_cliente)) {
          if (meta.status === "agente") outAgente++;
          else outBot++;
      ...
      if (range.gte) pqs += `&updated_at=gte.${range.gte}`;
      if (channel) pqs += `&channel=eq.${channel}`;
      const pendings: any[] = await sbApi(`/whatsapp_pending_contacts?${pqs}`);
      const activePending = new Set<string>();
      let newContacts = 0,
        promotions = 0;
      const byTriage: Record<string, number> = {};
      for (const p of pendings) {
        const msgs: PendingMsg[] = Array.isArray(p.messages) ? p.messages : [];
        let touched = false;
        for (const m of msgs) {
          if (!m.ts || !inRange(m.ts)) continue;
          touched = true;
          if (m.dir === "in") inbound++;
          else outBot++;
          tally(p.channel ?? "whatsapp", m.dir === "in" ? "in" : "out", m.ts);
        }
        if (touched) activePending.add(p.respond_contact_id);
        if (inRange(p.created_at)) {
          newContacts++;
          byTriage[p.tipo_contacto ?? "—"] = (byTriage[p.tipo_contacto ?? "—"] ?? 0) + 1;
        }
        if (p.status === "lead" && inRange(p.status_changed_at)) promotions++;
      }

      // ── Escalations (leads the bot handed to a human, active in period) ──
      let escQs = `bot_escalation_reason=not.is.null`;
      if (range.gte) escQs += `&whatsapp_last_inbound_at=gte.${range.gte}`;
      if (range.lte) escQs += `&whatsapp_last_inbound_at=lt.${range.lte}`;
      const escalations = await sbCount("leads", escQs).catch(() => 0);

      const totalOut = outBot + outAgente;
      const pct = (n: number, d: number) => (d > 0 ? ((n / d) * 100).toFixed(1) + "%" : "—");
      const dirTable = (m: Record<string, { in: number; out: number }>) =>
        Object.entries(m)
          .sort()
...
server.tool(
  ...
  "WhatsApp bot error & pipeline health report: bot_error_log grouped by fase × tipo with recent errors, webhook_outbox (target=whatsapp) status counts + recent failures with last_error, and stuck inbound-buffer rows (unprocessed >10 min).",
  {
  ...
  async ({ days, limit }) => {
    try {
      ...
      const [errors, outboxFailed, stuck] = await Promise.all([
        sbApi(
          `/bot_error_log?created_at=gte.${since}&order=created_at.desc&select=created_at,contact_key,fase,tipo,detalle&limit=1000`
        ) as Promise<any[]>,
        sbApi(
          `/webhook_outbox?target=eq.whatsapp&status=eq.failed&created_at=gte.${since}&order=created_at.desc&select=created_at,event_type,attempts,last_error&limit=${lim}`
        ) as Promise<any[]>,
        sbApi(
          `/whatsapp_inbound_buffer?processed=eq.false&created_at=lt.${encodeURIComponent(stuckBefore)}&order=created_at.asc&select=contact_key,created_at&limit=50`
        ) as Promise<any[]>,
      ]);
      ...
      const byFaseTipo: Record<string, number> = {};
      for (const e of errors) byFaseTipo[`${e.fase} / ${e.tipo}`] = (byFaseTipo[`${e.fase} / ${e.tipo}`] ?? 0) + 1;

      const verdict = (bad: number, warnAt = 1) => (bad === 0 ? "✅" : bad < warnAt ? "⚠️" : "🔴");
            ...
            text: [
              ...
              errors.length
                ? `\n## Errores recientes (${Math.min(errors.length, lim)})\n\n| Fecha | Fase | Tipo | Contacto | Detalle |\n|-------|------|------|----------|---------|\n${errors
                    ...
                    .map(
                      (e: any) =>
                        `| ${fmtTs(e.created_at)} | ${e.fase} | ${e.tipo} | ${clip(e.contact_key, 24)} | ${clip(e.detalle, 90)} |`
                    )
                    .join("\n")}`
                : "",
              outboxFailed.length
                ? `\n## Outbox fallidos recientes\n\n| Fecha | Evento | Intentos | Error |\n|-------|--------|----------|-------|\n${outboxFailed
                    .map(
                      (o: any) => `| ${fmtTs(o.created_at)} | ${o.event_type} | ${o.attempts} | ${clip(o.last_error, 90)} |`
                    )
                    .join("\n")}`
                : "",
            ]
