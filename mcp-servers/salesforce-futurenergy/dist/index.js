...
import { PHOTO_TO_DOCUMENT_MAP, VT_TO_SF_FIELD_MAP, ACCOUNT_EMAIL_FIELDS, } from "./schema.js";
...
function requireCredentials() {
    if (!hasCredentials()) {
        throw new Error("No Salesforce credentials. Set SF_CLIENT_ID and SF_CLIENT_SECRET in .mcp.json env.");
    }
}
...
async function sfDescribe(objectName) {
...
function formatRecord(record, indent = 2) {
...
// ────────────────────────────────────────────────────────────
server.tool("salesforce_describe_object", "Describe any Salesforce object live. Returns all fields with types, picklist values, required flags, and relationships. Use this to understand the structure of Lead, Opportunity, Account, Contact, or any custom object.", {
    object_name: z
        .string()
...
server.tool("salesforce_list_objects", "List Salesforce objects in the org. Filter by keyword or show only custom objects.", {
    ...
    try {
        ...
        if (filter) {
            ...
            objects = objects.filter((o) => o.name.toLowerCase().includes(kw) || (o.label ?? "").toLowerCase().includes(kw));
        }
        const lines = objects.map((o) => `- **${o.label}** (\`${o.name}\`)${o.custom ? " [Custom]" : ""}`);
        return {
            content: [
...
server.tool("salesforce_get_field", "Get detailed info about a specific field: type, picklist values, required, formula, help text, references.", {
    ...
    try {
        requireCredentials();
        const metadata = await sfDescribe(object_name);
        const field = (metadata.fields ?? []).find((f) => f.name.toLowerCase() === field_name.toLowerCase());
        if (!field)
            return {
        ...
        if (field.inlineHelpText)
            lines.push(`- **Help text:** ${field.inlineHelpText}`);
        if (field.calculatedFormula)
            lines.push(`- **Formula:** \`${field.calculatedFormula}\``);
        if (field.defaultValueFormula)
            lines.push(`- **Default:** \`${field.defaultValueFormula}\``);
        if (field.referenceTo?.length > 0)
            lines.push(`- **References:** ${field.referenceTo.join(", ")}`);
        if (field.picklistValues?.length > 0) {
            const vals = field.picklistValues
                .filter((v) => v.active)
                .map((v) => `  - \`${v.value}\`${v.label !== v.value ? ` (label: ${v.label})` : ""}${v.defaultValue ? " **default**" : ""}`)
                .join("\n");
...
server.tool("salesforce_search_fields", "Search for fields by keyword on any Salesforce object. Searches names, labels, and help text.", {
    ...
    try {
        ...
        const matches = (metadata.fields ?? []).filter((f) => f.name.toLowerCase().includes(kw) ||
        ...
        const lines = matches.map((f) => {
            ...
            if (f.picklistValues?.length > 0) {
                const vals = f.picklistValues
                    ...
                    .slice(0, 8)
                    .map((v) => v.value);
                const more = f.picklistValues.filter((v) => v.active).length > 8 ? "..." : "";
                line += `\n  Values: ${vals.map((v) => `\`${v}\``).join(", ")}${more}`;
            }
            return line;
        });
        return {
...
// DATA QUERY TOOLS
// ────────────────────────────────────────────────────────────
server.tool("salesforce_search", "Search Salesforce across objects using SOSL. Finds Leads, Opportunities, Accounts, Contacts, etc. by name, email, phone, or any text.", {
    ...
    try {
        ...
        const lines = records.map((r, i) => `${i + 1}. **${r.Name}** (${r.attributes?.type}) — \`${r.Id}\``);
        return {
...
server.tool("salesforce_query", "Run a SOQL query. Use this for flexible data retrieval: SELECT fields FROM object WHERE conditions. Returns up to 200 records.", {
...
server.tool("salesforce_get_project", "Get a combined project overview in one call: Opportunity details + Account contact info + Documento__c checklist. Pass either a project_id (e.g. EFU-00514-1) or a Salesforce opportunity_id.", {
    project_id: z
        .string()
        .optional()
    ...
    try {
        ...
        if (opp.AccountId) {
            try {
                account = cleanRecord(await sfApi(`/sobjects/Account/${opp.AccountId}?fields=Id,Name,PersonEmail,PersonMobilePhone,Phone,Email_Facturaci_n__c`));
            }
            catch {
                // PersonEmail may not exist on non-Person Accounts; retry without it
                try {
                    account = cleanRecord(await sfApi(`/sobjects/Account/${opp.AccountId}?fields=Id,Name,PersonMobilePhone,Phone,Email_Facturaci_n__c`));
                }
                catch {
        ...
        let docs = [];
        try {
            const docData = await sfApi(`/query/?q=${encodeURIComponent(`SELECT Id, Name, Requerido__c, Documento_Agregado__c FROM Documento__c WHERE Oportunidad__c = '${opp.Id}' ORDER BY Name`)}`);
        ...
        const docLines = docs.length > 0
            ? [
                ...
                ...docs.map((d) => `- **${d.Name}** | Required: ${d.Requerido__c ? "Yes" : "No"} | Files: ${d.Documento_Agregado__c ? "Yes" : "No"} | \`${d.Id}\``),
            ].join("\n")
...
server.tool("salesforce_get_record", "Get a single Salesforce record by object type and ID. Works for any object: Lead, Opportunity, Account, Contact, User, Documento__c, etc.", {
...
server.tool("salesforce_get_related", "Get related records for a parent record. E.g., all Opportunities for an Account, all Activities for a Lead, all Documents for an Opportunity.", {
...
server.tool("salesforce_recent_records", "Get recent records for any object type. Useful for seeing latest Leads, Opportunities, etc.", {
    ...
    try {
        ...
        if (keys.length <= 6) {
            ...
            const rows = records.map((r) => `| ${keys.map((k) => String(r[k] ?? "")).join(" | ")} |`);
            return {
                content: [
                    {
                        type: "text",
...
server.tool("salesforce_count", "Count records for any object, optionally filtered. Quick way to get stats.", {
    object_name: z.string().describe("Object API name (Lead, Opportunity, Account, etc.)"),
...
// DOCUMENT TOOLS
// ────────────────────────────────────────────────────────────
server.tool("salesforce_get_documents", "Get Documento__c records for an Opportunity. Shows which documents exist, are required, and have files.", {
    opportunity_id: z.string().describe("Salesforce Opportunity ID"),
}, async ({ opportunity_id }) => {
    try {
        requireCredentials();
        const data = await sfApi(`/query/?q=${encodeURIComponent(`SELECT Id, Name, Requerido__c, Documento_Agregado__c FROM Documento__c WHERE Oportunidad__c = '${opportunity_id}' ORDER BY Name`)}`);
        const docs = (data.records ?? []).map(cleanRecord);
        if (docs.length === 0)
            return { content: [{ type: "text", text: "No Documento__c records found." }] };
        const lines = docs.map((d) => `- **${d.Name}** | Required: ${d.Requerido__c ? "Yes" : "No"} | Files: ${d.Documento_Agregado__c ? "Yes" : "No"} | \`${d.Id}\``);
        return {
...
server.tool("salesforce_get_document_files", "Get files (ContentDocumentLinks) attached to a record. Works for Documento__c, Opportunity, Account, or any record.", {
    record_id: z.string().describe("Record ID to get files for (Documento__c ID, Opportunity ID, etc.)"),
}, async ({ record_id }) => {
    try {
        requireCredentials();
        const data = await sfApi(`/query/?q=${encodeURIComponent(`SELECT ContentDocumentId, ContentDocument.Title, ContentDocument.FileType, ContentDocument.FileExtension, ContentDocument.ContentSize, ContentDocument.CreatedDate FROM ContentDocumentLink WHERE LinkedEntityId = '${record_id}'`)}`);
...
server.tool("salesforce_download_file", "Download a file from Salesforce by ContentDocumentId and save it locally. Returns the local file path.", {
    ...
    try {
        ...
        const versionData = await sfApi(`/query/?q=${encodeURIComponent(`SELECT Id, Title, FileExtension, ContentSize, VersionData FROM ContentVersion WHERE ContentDocumentId = '${content_document_id}' AND IsLatest = true LIMIT 1`)}`);
...
// KPI & ANALYTICS TOOLS
// ────────────────────────────────────────────────────────────
server.tool("salesforce_kpis", "Get a full KPI dashboard for Future Energy in one call. Returns pipeline by stage, monthly trends, conversion metrics, project stats, and more. Perfect for management questions like 'how are we doing?' or 'show me the numbers'.", {
    period: z
    ...
    try {
        ...
        const sections = [`# Future Energy KPIs — ${timePeriod.replace(/_/g, " ").toUpperCase()}\n`];
        // ── Pipeline by Stage ─────────────────────────────────
        if (section === "all" || section === "pipeline") {
            const stageData = await sfApi(`/query/?q=${encodeURIComponent(`SELECT StageName, COUNT(Id) cnt FROM Opportunity${whereDate} GROUP BY StageName ORDER BY COUNT(Id) DESC`)}`);
            const stageRecords = (stageData.records ?? []).map(cleanRecord);
            const totalOpps = stageRecords.reduce((sum, r) => sum + r.cnt, 0);
            const stageLines = stageRecords.map((r) => `| ${r.StageName} | ${r.cnt} | ${totalOpps > 0 ? ((r.cnt / totalOpps) * 100).toFixed(1) : 0}% |`);
            sections.push(`## Pipeline by Stage (${totalOpps} total)\n\n| Stage | Count | % |\n|-------|-------|---|\n${stageLines.join("\n")}`);
            // Amount by stage (if Amount is populated)
            try {
                const amountData = await sfApi(`/query/?q=${encodeURIComponent(`SELECT StageName, SUM(Amount) total, AVG(Amount) avg FROM Opportunity WHERE Amount != null${andDate ? andDate : whereDate ? andDate : ""} GROUP BY StageName ORDER BY SUM(Amount) DESC`)}`);
                const amountRecords = (amountData.records ?? []).map(cleanRecord);
                if (amountRecords.length > 0 && amountRecords.some((r) => r.total > 0)) {
                    const amtLines = amountRecords
                        .filter((r) => r.total > 0)
                        .map((r) => `| ${r.StageName} | $${Number(r.total).toLocaleString()} | $${Number(r.avg).toLocaleString(undefined, { maximumFractionDigits: 0 })} |`);
                    sections.push(`## Revenue Pipeline\n\n| Stage | Total Amount | Avg Deal |\n|-------|-------------|----------|\n${amtLines.join("\n")}`);
                }
            }
            catch { }
        }
        ...
        if (section === "all" || section === "conversion") {
            const wonData = await sfApi(`/query/?q=${encodeURIComponent(`SELECT COUNT() FROM Opportunity WHERE IsWon = true${andDate}`)}`);
            const lostData = await sfApi(`/query/?q=${encodeURIComponent(`SELECT COUNT() FROM Opportunity WHERE IsClosed = true AND IsWon = false${andDate}`)}`);
            const totalData = await sfApi(`/query/?q=${encodeURIComponent(`SELECT COUNT() FROM Opportunity${whereDate}`)}`);
            ...
            sections.push(`## Conversion Metrics\n\n| Metric | Value |\n|--------|-------|\n| Total Opportunities | ${total} |\n| Won (Closed-Won) | ${won} |\n| Lost (Closed-Lost) | ${lost} |\n| Open (In Progress) | ${open} |\n| Win Rate | ${winRate}% |`);
        }
        // ── Project Stats ─────────────────────────────────────
        if (section === "all" || section === "projects") {
            // Roof type distribution
            try {
                const roofData = await sfApi(`/query/?q=${encodeURIComponent(`SELECT Tipo_de_techo__c, COUNT(Id) cnt FROM Opportunity WHERE Tipo_de_techo__c != null${andDate} GROUP BY Tipo_de_techo__c ORDER BY COUNT(Id) DESC`)}`);
                const roofRecords = (roofData.records ?? []).map(cleanRecord);
                if (roofRecords.length > 0) {
                    const roofLines = roofRecords.map((r) => `| ${r.Tipo_de_techo__c} | ${r.cnt} |`);
                    sections.push(`## Roof Type Distribution\n\n| Type | Count |\n|------|-------|\n${roofLines.join("\n")}`);
                }
            }
            catch { }
            // Project type distribution
            try {
                const typeData = await sfApi(`/query/?q=${encodeURIComponent(`SELECT Tipo_De_Proyecto__c, COUNT(Id) cnt FROM Opportunity WHERE Tipo_De_Proyecto__c != null${andDate} GROUP BY Tipo_De_Proyecto__c ORDER BY COUNT(Id) DESC`)}`);
                const typeRecords = (typeData.records ?? []).map(cleanRecord);
                if (typeRecords.length > 0) {
                    const typeLines = typeRecords.map((r) => {
                        // SF may return the field under various key casings
                        const typeVal = r.Tipo_De_Proyecto__c ?? r.tipo_de_proyecto__c ?? Object.values(r).find((v) => typeof v === "string" && v !== "") ?? "Unknown";
                        return `| ${typeVal} | ${r.cnt} |`;
                    });
                    sections.push(`## Project Type Distribution\n\n| Type | Count |\n|------|-------|\n${typeLines.join("\n")}`);
                }
            }
            catch { }
            // Documents completion
            try {
                const docsTotal = await sfApi(`/query/?q=${encodeURIComponent(`SELECT COUNT() FROM Documento__c WHERE Requerido__c = true`)}`);
                const docsComplete = await sfApi(`/query/?q=${encodeURIComponent(`SELECT COUNT() FROM Documento__c WHERE Requerido__c = true AND Documento_Agregado__c = true`)}`);
        ...
        if (section === "all" || section === "owners") {
            try {
                const ownerData = await sfApi(`/query/?q=${encodeURIComponent(`SELECT Owner.Name, COUNT(Id) cnt FROM Opportunity${whereDate} GROUP BY Owner.Name ORDER BY COUNT(Id) DESC LIMIT 15`)}`);
                const ownerRecords = (ownerData.records ?? []).map(cleanRecord);
                if (ownerRecords.length > 0) {
                    const ownerLines = ownerRecords.map((r) => `| ${r.Name ?? "Unknown"} | ${r.cnt} |`);
                    sections.push(`## Opportunities by Owner\n\n| Owner | Count |\n|-------|-------|\n${ownerLines.join("\n")}`);
                }
            }
            catch { }
            // Won by owner
            try {
                const wonOwnerData = await sfApi(`/query/?q=${encodeURIComponent(`SELECT Owner.Name, COUNT(Id) cnt FROM Opportunity WHERE IsWon = true${andDate} GROUP BY Owner.Name ORDER BY COUNT(Id) DESC LIMIT 15`)}`);
                const wonOwnerRecords = (wonOwnerData.records ?? []).map(cleanRecord);
                if (wonOwnerRecords.length > 0) {
                    const wonOwnerLines = wonOwnerRecords.map((r) => `| ${r.Name ?? "Unknown"} | ${r.cnt} |`);
                    sections.push(`## Closed-Won by Owner\n\n| Owner | Won |\n|-------|-----|\n${wonOwnerLines.join("\n")}`);
                }
            }
            catch { }
        }
        ...
        if (section === "all" || section === "leads") {
            // Total leads
            try {
                const leadTotal = await sfApi(`/query/?q=${encodeURIComponent(`SELECT COUNT() FROM Lead${whereDate}`)}`);
                sections.push(`## Leads Overview\n\n**Total Leads (${timePeriod.replace(/_/g, " ")}):** ${leadTotal.totalSize ?? 0}`);
            }
            catch { }
            // Leads by Status
            try {
                const statusData = await sfApi(`/query/?q=${encodeURIComponent(`SELECT Status, COUNT(Id) cnt FROM Lead${whereDate} GROUP BY Status ORDER BY COUNT(Id) DESC`)}`);
                const statusRecords = (statusData.records ?? []).map(cleanRecord);
                if (statusRecords.length > 0) {
                    const totalLeads = statusRecords.reduce((sum, r) => sum + r.cnt, 0);
                    const statusLines = statusRecords.map((r) => `| ${r.Status ?? "—"} | ${r.cnt} | ${totalLeads > 0 ? ((r.cnt / totalLeads) * 100).toFixed(1) : 0}% |`);
                    sections.push(`## Leads by Status\n\n| Status | Count | % |\n|--------|-------|---|\n${statusLines.join("\n")}`);
                }
            }
            catch { }
            // Leads by Source
            try {
                const sourceData = await sfApi(`/query/?q=${encodeURIComponent(`SELECT LeadSource, COUNT(Id) cnt FROM Lead WHERE LeadSource != null${andDate} GROUP BY LeadSource ORDER BY COUNT(Id) DESC LIMIT 15`)}`);
                const sourceRecords = (sourceData.records ?? []).map(cleanRecord);
                if (sourceRecords.length > 0) {
                    const sourceLines = sourceRecords.map((r) => `| ${r.LeadSource} | ${r.cnt} |`);
                    sections.push(`## Leads by Source\n\n| Source | Count |\n|--------|-------|\n${sourceLines.join("\n")}`);
                }
            }
            catch { }
            // Leads by Tipo_Lead__c (Hot/Warm/Cold)
            try {
                const tipoData = await sfApi(`/query/?q=${encodeURIComponent(`SELECT Tipo_Lead__c, COUNT(Id) cnt FROM Lead WHERE Tipo_Lead__c != null${andDate} GROUP BY Tipo_Lead__c ORDER BY COUNT(Id) DESC`)}`);
                const tipoRecords = (tipoData.records ?? []).map(cleanRecord);
                if (tipoRecords.length > 0) {
                    const tipoLines = tipoRecords.map((r) => `| ${r.Tipo_Lead__c} | ${r.cnt} |`);
                    sections.push(`## Lead Quality (Tipo de Lead)\n\n| Type | Count |\n|------|-------|\n${tipoLines.join("\n")}`);
                }
            }
            catch { }
            // Lead conversion
            try {
                const convertedData = await sfApi(`/query/?q=${encodeURIComponent(`SELECT COUNT() FROM Lead WHERE IsConverted = true${andDate}`)}`);
                const unconvertedData = await sfApi(`/query/?q=${encodeURIComponent(`SELECT COUNT() FROM Lead WHERE IsConverted = false${andDate}`)}`);
                ...
                sections.push(`## Lead Conversion\n\n| Metric | Value |\n|--------|-------|\n| Total Leads | ${totalLeads} |\n| Converted | ${converted} |\n| Not Converted | ${unconverted} |\n| Conversion Rate | ${convRate}% |`);
            }
            catch { }
            // Leads by Owner
            try {
                const leadOwnerData = await sfApi(`/query/?q=${encodeURIComponent(`SELECT Owner.Name, COUNT(Id) cnt FROM Lead${whereDate} GROUP BY Owner.Name ORDER BY COUNT(Id) DESC LIMIT 15`)}`);
                const leadOwnerRecords = (leadOwnerData.records ?? []).map(cleanRecord);
                if (leadOwnerRecords.length > 0) {
                    const leadOwnerLines = leadOwnerRecords.map((r) => `| ${r.Name ?? "Unknown"} | ${r.cnt} |`);
                    sections.push(`## Leads by Owner\n\n| Owner | Count |\n|-------|-------|\n${leadOwnerLines.join("\n")}`);
                }
            }
            catch { }
            // Leads by Project Type
            try {
                const leadProjData = await sfApi(`/query/?q=${encodeURIComponent(`SELECT Tipo_De_Proyecto__c, COUNT(Id) cnt FROM Lead WHERE Tipo_De_Proyecto__c != null${andDate} GROUP BY Tipo_De_Proyecto__c ORDER BY COUNT(Id) DESC`)}`);
                const leadProjRecords = (leadProjData.records ?? []).map(cleanRecord);
                if (leadProjRecords.length > 0) {
                    const leadProjLines = leadProjRecords.map((r) => {
                        const typeVal = r.Tipo_De_Proyecto__c ?? Object.values(r).find((v) => typeof v === "string" && v !== "") ?? "Unknown";
                        return `| ${typeVal} | ${r.cnt} |`;
                    });
                    sections.push(`## Leads by Project Type\n\n| Type | Count |\n|------|-------|\n${leadProjLines.join("\n")}`);
                }
            }
            catch { }
            // Disqualification reasons
            try {
                const dqData = await sfApi(`/query/?q=${encodeURIComponent(`SELECT Descalificacion_Opciones__c, COUNT(Id) cnt FROM Lead WHERE Descalificacion_Opciones__c != null${andDate} GROUP BY Descalificacion_Opciones__c ORDER BY COUNT(Id) DESC LIMIT 10`)}`);
                const dqRecords = (dqData.records ?? []).map(cleanRecord);
                if (dqRecords.length > 0) {
                    const dqLines = dqRecords.map((r) => `| ${r.Descalificacion_Opciones__c} | ${r.cnt} |`);
                    sections.push(`## Top Disqualification Reasons\n\n| Reason | Count |\n|--------|-------|\n${dqLines.join("\n")}`);
                }
            }
            catch { }
        }
...
server.tool("salesforce_aggregate", "Run aggregate queries on Salesforce data without writing SOQL. Group by any field and get COUNT, SUM, AVG, MIN, MAX. Great for quick stats and breakdowns.", {
    ...
    try {
        ...
        const getGroupVal = (r) => {
            ...
            const simpleKey = group_by.includes(".")
                ? group_by.split(".").pop()
                : group_by;
            if (r[simpleKey] !== undefined)
                return String(r[simpleKey]);
            if (r[group_by] !== undefined)
                return String(r[group_by]);
            // For function expressions like CALENDAR_MONTH(CreatedDate), SF returns expr0
            for (const key of Object.keys(r)) {
                if (key.startsWith("expr") && key !== "val")
                    return String(r[key]);
            }
...
// MAPPING REFERENCE
// ────────────────────────────────────────────────────────────
server.tool("salesforce_field_mappings", "Show Pronto Resolver <-> Salesforce field mappings: Visita Tecnica -> Opportunity, photo categories -> documents, email field priority.", {
    mapping_type: z
    ...
    if (mapping_type === "visita_to_opportunity" || mapping_type === "all") {
        const lines = Object.entries(VT_TO_SF_FIELD_MAP).map(([vt, sf]) => `| ${vt} | \`${sf}\` |`);
        sections.push(`## Visita Tecnica -> Opportunity\n\n| VT Field | SF Field |\n|----------|----------|\n${lines.join("\n")}`);
    }
    if (mapping_type === "photo_to_document" || mapping_type === "all") {
        const lines = Object.entries(PHOTO_TO_DOCUMENT_MAP).map(([cat, doc]) => `| ${cat} | ${doc} |`);
        sections.push(`## Photo -> Documento__c\n\n| Category | Document |\n|----------|----------|\n${lines.join("\n")}`);
    }
    if (mapping_type === "email_fields" || mapping_type === "all") {
        sections.push(`## Account Email Priority\n\n${ACCOUNT_EMAIL_FIELDS.map((f, i) => `${i + 1}. \`${f}\``).join("\n")}`);
    }
...
// ────────────────────────────────────────────────────────────
server.prompt("futurenergy_context", "Essential context about Future Energy's Salesforce org. Use this to understand the data model, key objects, stages, and how to answer questions about the solar installation business.", () => ({
    messages: [
        {
            role: "user",
            content: {
...
server.prompt("kpi_dashboard", "Quick prompt to get a full KPI dashboard. Just select this prompt and Claude will fetch all key metrics.", () => ({
    messages: [
        {
            role: "user",
            content: {
...
server.prompt("pipeline_analysis", "Analyze the sales pipeline with stage breakdown, bottlenecks, and conversion insights.", { period: z.string().optional().describe("Time period: this_month, this_quarter, this_year, etc.") }, ({ period }) => ({
    messages: [
        {
            role: "user",
            content: {
